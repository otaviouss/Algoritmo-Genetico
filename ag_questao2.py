...
def inicializacao(tam_pop, min, max):
    ...
    for i in range(tam_pop):
        individuo = []
        for j in range(40):
            p_i = random.uniform(min[j], max[j])
            individuo.append(p_i)
        vi.append(individuo)
...
def atualiza_melhor(pais, melhor, min, dados_objetivo):
    ...
    for j in range(len(pais_ordenados)):
        ...
        if(fit_pai < calcular_funcao_individuo(melhor, min, dados_objetivo) and restricao(fit_pai) < 0 ):
...
def algoritmo_genetico():
    ...
    while True:
        ...
        print("Valor: ",calcular_funcao_individuo(novo_melhor, min, dados_objetivo),  " Melhor: ", novo_melhor, " Geração: ", cp)
        ...
        if(cp == n_geracoes):
            print(melhor)
            print(calcular_funcao_individuo(melhor, min, dados_objetivo))
            break
